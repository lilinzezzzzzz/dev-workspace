...
- **同步目标**: 将同目录下的 `AGENTS.md` 同步到 `~/.codex/AGENTS.md` 和 `~/.qoder/AGENTS.md`
- **权限设置**: 目标文件权限设置为 `0644`（所有者可读写，其他用户只读）
- **完整性校验**: 使用 `sha256sum` 对源文件和目标文件进行哈希校验，确保复制完整
- **错误处理**: 如果源文件不存在或校验失败，脚本会报错退出
- **依赖要求**: 需要系统安装 `sha256sum` 工具
