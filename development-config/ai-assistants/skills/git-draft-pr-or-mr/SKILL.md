...
description: 基于明确指定的基础分支或基础 ref 与实际 git diff 生成 PR/MR 标题和描述。用于用户要求“写 PR 标题”“生成 MR 描述”“基于 dev、main、master、origin/main 或其他 base branch 总结当前分支改动”“整理可直接粘贴到 GitHub/GitLab 的合并说明”等场景。默认将未限定的基础分支名解析为对应的 remote-tracking ref，而不是本地分支。输出默认偏业务视角，优先表达业务目标、用户影响和结果价值；只有在基础分支已指定或可被用户明确确认时使用，若未指定基础分支，先询问，不要自行猜测。
---

# Git Draft PR Or MR

Use this skill to draft a PR or MR title and description from the actual git diff between an explicit base ref and `HEAD`, with business outcome and user impact as the primary narrative.
...
7. Prefer business framing over implementation framing. Lead with the business problem solved, user or operator impact, and expected outcome; use technical details only when they are required for precision.
...
3. Identify the dominant story.
   - Summarize the primary business goal, user-facing or operator-facing outcome, and expected value.
   - Call out important guardrails, compatibility constraints, or contract changes.
   - Separate core business changes from supporting tests, cleanup, or enabling implementation details.
...
5. Draft the description.
   ...
   - Focus on why the change matters, what business problem it solves, what changed, and how it was verified.
   - Describe implementation details only to the extent needed to explain impact, scope, constraints, or release risk.
